...
void USART_Enable(void) {
    SERCOM_USART_Enable(SERCOM1);
}

void USART_WriteData(uint8_t* data, const uint8_t size) {
    SERCOM_USART_WriteData(SERCOM1, data, size);
}

uint16_t USART_Available(void) {
    return SERCOM_USART_Available(SERCOM1);
}

uint8_t USART_Read(void) {
    return SERCOM_USART_Read(SERCOM1);
}

void USART_FlushOutput(void) {
    SERCOM_USART_FlushOutput(SERCOM1);
}
